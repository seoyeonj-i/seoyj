...
AAPL=pd.read_csv(r"C:\Users\82105\Documents\GitHub\seoyj\python\data\AAPL.csv",index_col='Date')
AMZN=pd.read_csv("rC:\Users\82105\Documents\GitHub\seoyj\python\data\AAPL.csv",index_col='Date')
BND=Pd.read_csv("rC:\Users\82105\Documents\GitHub\seoyj\python\data\AAPL.csv",index_col='Date')

class Invest:
    # 생성자함수 : CLASS를 생성할때 최초로 한번 실행이되는 함수
    def __init__(
            self,
            _df,
            _col='Adj Close',
        ...
        # _start, _end는 시계열 데이터로 변경해서 객체변수에 대입
        try:
    ...
    def bollinger(self, _cnt=20):
        # 밴드를 생성하는 함수 호출
        band_df=boll.create_band(self.df, self.col, self.start, self.end, _cnt)
        # 거래내역을 추가하는 함수 호출
