...
mydb=database.MyDB1(_host='172.16.12.155',
                    _port=3306,
                    _user='ubion',
                    _password='1234',
                    _database='ubion10'
...
def login():
    ...
    # data에서 user table에서 유저가 입력한 아이디 패스워드가 존재하는가?
...
def login2():
    ...
    if result:
        ...
        # 로그인이 성공했을때 새로운 페이지를 보여주고 해당페이지에 로그인을 한 유저의 이름과 인사문구 출력
...
def view_corona():
    ...
    print(cols)
    print(val)
    return render_template('corona.html',columns=cols, values=val,c_cnt=colsCnt,v_cnt=valCnt,axis_x=x,axis_y=y)
...
# http://127.0.0.1:5000/corona?serviceKey=aaa&_limit=1
# 웹서버를 시작
app.run(port=5000, debug=True)

# 폴더 경로복사후 명령프롬프트 cd {경로입력}-> main.py